...
class RuleEngine:
    ...
    def validate_nomination(
        ...
        if nominator in today_nominators:
            return RuleValidation(
                False, f"{nominator} has already made a nomination today"
            )

        # Character-specific restrictions
        char_validation = self._validate_character_nomination_restrictions(
            nominator_player, nominee_player, game_state
        )
    ...
    def validate_vote(
        self, voter: str, nominee: str, game_state: GameState
    ) -> RuleValidation:
        ...
        if voter in current_nomination.voters:
            return RuleValidation(
                False, f"{voter} has already voted on this nomination"
            )

        # Character-specific voting restrictions
        char_validation = self._validate_character_voting_restrictions(
            voter_player, nominee_player, game_state
        )
        if not char_validation.is_valid:
            return char_validation
    ...
    def validate_execution(
        self, nominee: str, vote_count: int, game_state: GameState
    ) -> RuleValidation:
        ...
        if vote_count < threshold:
            return RuleValidation(
                False, f"Insufficient votes: {vote_count}/{threshold} required"
            )
        ...
        char_validation = self._validate_character_execution_restrictions(
            nominee_player, game_state
        )
        if not char_validation.is_valid:
            return char_validation
    ...
    def validate_night_action(
        ...
        if game_state.phase not in [GamePhase.FIRST_NIGHT, GamePhase.NIGHT]:
            return RuleValidation(
                False, "Night actions only allowed during night phase"
            )
        ...
        if (
            player.is_drunk or player.is_poisoned
        ) and not self._character_acts_when_drunk_poisoned(character):
            return RuleValidation(
                False, f"{player.name} is drunk/poisoned and cannot act normally"
            )
        ...
        if target:
            target_validation = self._validate_night_action_target(
                character, player, target, game_state
            )
    ...
    def validate_win_condition(
        ...
        evil_wins, evil_reason = self._check_evil_victory_conditions(
            game_state, alive_players
        )
        if evil_wins:
    ...
    def apply_execution(self, nominee: str, game_state: GameState) -> List[str]:
        ...
        death_effects = self._trigger_death_abilities(
            nominee_player, game_state, "execution"
        )
        effects.extend(death_effects)

        # Check for special execution effects
        special_effects = self._apply_special_execution_effects(
            nominee_player, game_state
        )
    ...
    def apply_demon_kill(
        self, demon_player: Player, target: str, game_state: GameState
    ) -> List[str]:
        ...
        death_effects = self._trigger_death_abilities(
            target_player, game_state, "demon"
        )
        effects.extend(death_effects)

        return effects

    def _validate_character_nomination_restrictions(
        self, nominator: Player, nominee: Player, game_state: GameState
    ) -> RuleValidation:
        ...
        if nominator.character == "Butler":
            ...
            if master and not self._has_master_nominated_today(master, game_state):
                return RuleValidation(
                    False, "Butler must wait for their Master to nominate first"
                )

        return RuleValidation(True, "No character restrictions")

    def _validate_character_voting_restrictions(
        self, voter: Player, nominee: Player, game_state: GameState
    ) -> RuleValidation:
        ...
        if voter.character == "Butler":
            ...
            if master:
                current_nomination = self._get_current_nomination(
                    nominee.name, game_state
                )
                if current_nomination and master.name not in current_nomination.voters:
                    return RuleValidation(
                        False, "Butler can only vote if their Master votes"
                    )

        # Organ Grinder - all votes are secret
        if any(
            p.character == "Organ Grinder" and p.is_alive() for p in game_state.players
        ):
    ...
    def _validate_character_execution_restrictions(
        self, nominee: Player, game_state: GameState
    ) -> RuleValidation:
        ...
        if nominee.character == "Saint" and nominee.team == Team.GOOD:
            return RuleValidation(
                True,
                "WARNING: Executing the Saint will cause good team to lose!",
                "confirm_saint_execution",
            )

        return RuleValidation(True, "No execution restrictions")

    def _check_good_victory_conditions(
        self, game_state: GameState, alive_players: List[Player]
        ...
        demons = [
            ...
            if self._is_demon(p.character) and p.status == PlayerStatus.DEAD
        ]
        if demons:
    ...
    def _check_evil_victory_conditions(
        self, game_state: GameState, alive_players: List[Player]
    ) -> Tuple[bool, Optional[str]]:
        ...
        non_traveler_alive = [
            p for p in alive_players if p.status != PlayerStatus.TRAVELER
        ]
    ...
    def _trigger_death_abilities(
        self, dead_player: Player, game_state: GameState, death_type: str
    ) -> List[str]:
    ...
    def _apply_special_execution_effects(
        self, executed_player: Player, game_state: GameState
    ) -> List[str]:
    ...
    def _character_has_night_ability(
        self, character: str, is_first_night: bool
    ) -> bool:
        ...
        if is_first_night:
            return (
                character in first_night_characters
                or character in ongoing_night_characters
            )
        else:
    ...
    def _has_acted_this_night(self, player: Player, game_state: GameState) -> bool:
        ...
        for action in game_state.night_actions:
            if (
                ...
                and action.timestamp.date() == datetime.now().date()
            ):
                return True

        return False

    def _validate_night_action_target(
        self, character: str, player: Player, target: str, game_state: GameState
    ) -> RuleValidation:
    ...
    def _get_butler_master(
        self, butler: Player, game_state: GameState
    ) -> Optional[Player]:
    ...
    def _has_master_nominated_today(
        self, master: Player, game_state: GameState
    ) -> bool:
    ...
    def _get_current_nomination(
        self, nominee: str, game_state: GameState
    ) -> Optional[Nomination]:
...
class CharacterAbilityValidator:
    ...
    def _load_ability_rules(self) -> Dict[str, Dict]:
        ...
        return {
            ...
            "Soldier": {"passive": True, "demon_immune": True},
            "Mayor": {"passive": True, "win_condition": "no_execution_3_alive"},
        }

    def validate_ability_use(
        self, character: str, ability_context: Dict
    ) -> RuleValidation:
        ...
        if rules.get("first_night_only") and not ability_context.get("is_first_night"):
            return RuleValidation(
                False, f"{character} ability only works on first night"
            )
        ...
        if actual_targets != expected_targets:
            return RuleValidation(
                False, f"{character} requires exactly {expected_targets} targets"
            )
