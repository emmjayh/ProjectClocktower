...
class ModelDownloader:
    ...
    async def download_whisper_model(self, model_size: str = "base") -> bool:
        ...
        try:
            ...
            self.logger.info(f"Loading Whisper {model_size} model (downloading if needed)...")
    ...
    async def download_piper_voice(self, voice_name: str = "en_US-lessac-medium") -> bool:
            ...
            for chunk in response.iter_content(chunk_size=8192):
                if chunk:
                    ...
                    if total_size > 0:
                        ...
                        print(f"\rDownloading {path.name}: {progress:.1f}%", end="", flush=True)
    ...
    async def _install_piper(self) -> None:
        ...
        subprocess.run([
            "wget", "-O", str(piper_dir / "piper.tar.gz"), piper_url
        ], check=True)
...
class SpeechHandler:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.whisper_model = whisper.load_model(
                ...
                download_root=str(self.downloader.models_dir / f"whisper_{self.config.whisper_model}")
            )
    ...
    async def listen_for_command(self, keywords: List[str] = None, timeout: float = 30.0) -> Optional[str]:
    ...
    async def speak(self, text: str) -> bool:
        ...
        try:
            ...
            voice_path = self.downloader.models_dir / "piper" / self.config.tts_voice / f"{self.config.tts_voice}.onnx"
    ...
    async def _record_audio(self, timeout: float) -> Optional[bytes]:
        ...
        try:
            ...
            frames = []
            silence_count = 0
            max_silence = int(self.config.silence_duration * self.config.sample_rate / self.config.chunk_size)
