...
class VotingSystem:
    ...
    def can_nominate(self, nominator_name: str, nominee_name: str) -> Tuple[bool, str]:
        ...
        nominator_nominations = [n for n in self.nominations if n.nominator.lower() == nominator_name.lower()]
        if nominator_nominations:
            return False, "Already nominated someone today"
            
        # Check if nominee already nominated today
        nominee_nominations = [n for n in self.nominations if n.nominee.lower() == nominee_name.lower()]
        if nominee_nominations:
            return False, f"{nominee_name} already nominated today"
            
        return True, "Valid nomination"
        
    def create_nomination(self, nominator_name: str, nominee_name: str) -> Optional[Nomination]:
    ...
    def can_vote(self, player_name: str) -> Tuple[bool, str]:
        ...
        existing_vote = next((v for v in self.current_nomination.votes 
                            if v.player_name.lower() == player_name.lower()), None)
        if existing_vote:
...
class SimpleMajorityVoting:
    ...
    def start_vote(self, nominee: str) -> bool:
        ...
        player = next((p for p in self.players if p.name.lower() == nominee.lower()), None)
    ...
    def cast_vote(self, voter: str, vote: VoteType) -> bool:
        ...
        player = next((p for p in self.players if p.name.lower() == voter.lower()), None)
        if not player or not player.is_alive():
            return False
...
if __name__ == "__main__":
    ...
    print(f"Simple vote result: {yes} yes, {no} no → {'Execute' if execute else 'No execution'}")