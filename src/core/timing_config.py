...
class TimingManager:
    ...
    def check_timing(self, phase_name: str) -> Optional[str]:
        ...
        if elapsed > expected and (elapsed - expected) % self.config.reminder_intervals < 5:
    ...
    def get_pacing_recommendation(self, player_count: int, experience_level: str = "mixed") -> TimingConfig:
    ...
    async def narrate_phase_start(self, phase: str, context: dict = None) -> str:
        ...
        if phase == "night":
            narration = await self.narrator.narrate_night_phase(
                context.get("night_number", 1),
                context.get("alive_count", 0)
            )
        elif phase == "day_discussion":
            narration = await self.narrator.announce_deaths(
                context.get("deaths", [])
            )
        else:
