...
class MVPCharacterAbilities:
    ...
    def fortune_teller_ability(self, player: Player, target1: str, target2: str) -> AbilityResult:
    ...
    def washerwoman_ability(self, player: Player) -> AbilityResult:
        ...
        townsfolk = [p for p in self.game_state.players if self._is_townsfolk(p.character)]
        ...
        other_players = [p for p in self.game_state.players if p.name != chosen_townsfolk.name]
    ...
    def _get_neighbors(self, player: Player) -> Tuple[Optional[Player], Optional[Player]]:
        ...
        try:
            player_index = next(i for i, p in enumerate(alive_players) if p.name == player.name)
    ...
    def _handle_imp_starpass(self, imp_player: Player) -> AbilityResult:
        ...
        minions = [p for p in self.game_state.players 
    ...
    def _get_washerwoman_info(self, player: Player, actual_character: str) -> str:
        ...
        if player.is_drunk or player.is_poisoned:
            if random.random() < 0.3:  # 30% chance of true info
                ...
                townsfolk_chars = ["Washerwoman", "Chef", "Empath", "Fortune Teller", "Monk"]
...
class AbilityDispatcher:
    ...
    def execute_ability(self, character: str, player: Player, **kwargs) -> AbilityResult:
...
if __name__ == "__main__":
    ...
    # Test Fortune Teller
    ft_result = dispatcher.execute_ability("Fortune Teller", players[0], target1="Grace", target2="Frank")
