...
from ..ai.autonomous_config import AutonomousConfig, PRESET_CONFIGS, get_config_description
from ..ai.character_handlers import CharacterAbilityHandler
from ..core.game_state import GameState, Player, GamePhase
from ..core.timing_config import TimingManager, PacingStyle
...
class AutonomousControlPanel:
    ...
    def __init__(self, parent: tk.Widget):
        ...
        self.panel = ttk.LabelFrame(
            parent,
            text="🤖 Autonomous AI Storyteller",
            style="Dark.TFrame"
        )
    ...
    def _create_config_tab(self):
        ...
        self.preset_desc_label = ttk.Label(
            preset_frame,
            text=get_config_description("standard"),
            foreground="#888888"
        )
        ...
        ttk.Label(autonomy_frame, text="Autonomy Level:").grid(row=0, column=0, sticky="w", padx=5)
        ...
        ttk.Label(info_frame, text="Truth Rate (Healthy):").grid(row=0, column=0, sticky="w", padx=5)
        ...
        ttk.Label(info_frame, text="Drama Factor:").grid(row=1, column=0, sticky="w", padx=5)
        self.drama_var = tk.DoubleVar(value=0.15)
        drama_scale = ttk.Scale(
            info_frame,
            from_=0.0,
            to=0.5,
            variable=self.drama_var,
            orient="horizontal"
        )
        ...
        ttk.Label(narration_frame, text="Drama Level:").grid(row=0, column=0, sticky="w", padx=5)
        ...
        ttk.Label(narration_frame, text="Length:").grid(row=1, column=0, sticky="w", padx=5)
        ...
        ttk.Checkbutton(
            narration_frame,
            text="Gothic atmosphere",
            variable=self.gothic_var
        ).grid(row=2, column=0, columnspan=2, sticky="w", padx=5, pady=5)
        ...
        ttk.Label(speech_frame, text="Sensitivity:").grid(row=0, column=0, sticky="w", padx=5)
        ...
        ttk.Label(speech_frame, text="Action Timeout (sec):").grid(row=1, column=0, sticky="w", padx=5)
    ...
    def _create_status_tab(self):
        ...
        speech_status_frame = ttk.LabelFrame(self.status_frame, text="Speech Recognition")
    ...
    def _create_decision_log_tab(self):
        ...
        ttk.Button(
            controls_frame,
            text="🔄 Refresh",
            command=self._refresh_decision_log
        ).pack(side="left", padx=5)
        
        ttk.Button(
            controls_frame,
            text="💾 Export Log",
            command=self._export_decisions
        ).pack(side="left", padx=5)
        
        ttk.Button(
            controls_frame,
            text="🗑️ Clear Log",
            command=self._clear_decision_log
        ).pack(side="left", padx=5)
        ...
        # Scrollbar for tree
        tree_scroll = ttk.Scrollbar(self.log_frame, orient="vertical", command=self.decision_tree.yview)
    ...
    def _on_preset_change(self, event=None):
        ...
        self.config = PRESET_CONFIGS[preset_name].copy() if preset_name in PRESET_CONFIGS else AutonomousConfig()
    ...
    def _emergency_stop(self):
        ...
        if messagebox.askyesno("Emergency Stop", "This will immediately stop all AI operations. Continue?"):
            self._stop_autonomous_mode()
    ...
    def _manual_override(self):
        ...
        if self.is_running and self.config.human_override:
            ...
            messagebox.showinfo("Manual Override", "AI operations paused. You now have manual control.")
        else:
            messagebox.showwarning("Override Denied", "Manual override not allowed in current configuration.")
    ...
    def _refresh_decision_log(self):
        ...
        if self.autonomous_storyteller and self.autonomous_storyteller.game_context:
            for info in self.autonomous_storyteller.game_context.information_history:
                self.decision_tree.insert("", "end",
                    text=f"N{info.night_number}",
                    values=(
                        info.timestamp.strftime("%H:%M"),
                        info.player_name,
                        info.character,
                        info.information[:50] + "..." if len(info.information) > 50 else info.information,
