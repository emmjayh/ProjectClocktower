...
This is a one-time download that will take a few minutes."""
    ...
    def download_models(self):
        ...
        try:
            # Update status
            self.root.after(0, self.status_label.config, {"text": "Downloading Whisper model..."})
            self.root.after(0, self.log, "📥 Downloading Whisper speech recognition model...")
            
            # Download Whisper
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            success = loop.run_until_complete(self.downloader.download_whisper_model("base"))
            ...
            # Update status
            self.root.after(0, self.status_label.config, {"text": "Downloading Piper voice..."})
            self.root.after(0, self.log, "📥 Downloading Piper text-to-speech voice...")
            
            # Download Piper
            success = loop.run_until_complete(self.downloader.download_piper_voice("en_US-lessac-medium"))
    ...
    def download_complete(self):
        ...
        self.download_btn.config(text="Close", state=tk.NORMAL, command=self.root.destroy)
...
def check_first_run() -> bool:
    ...
    piper_exists = (models_dir / "piper" / "en_US-lessac-medium" / "en_US-lessac-medium.onnx").exists()
