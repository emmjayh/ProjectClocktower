...
"""
        ...
        for action in self.player_actions[-15:]:  # Last 15 actions
            context += f"{action.player_name}: {action.action_type} - {action.details}\n"
...
class SpeechParser:
    ...
    def parse_speech(self, text: str, speaker: str, game_state: GameState) -> Optional[PlayerAction]:
        ...
        for action_type, patterns in self.patterns.items():
            for pattern in patterns:
                ...
                if match:
                    return self._create_action(action_type, match, text, speaker, game_state)
    ...
    def _create_action(self, action_type: str, match, text: str, speaker: str, game_state: GameState) -> PlayerAction:
...
class AutonomousStoryteller:
    ...
    def __init__(self, speech_handler: Optional[SpeechHandler] = None, ai_storyteller=None):
    ...
    async def initialize(self) -> bool:
        ...
        if self.ai_storyteller:
            ...
            if not ai_success:
                self.logger.warning("AI model failed to load - using fallback decisions")
        else:
        ...
        if self.speech_handler:
            ...
            self.logger.warning("No speech handler provided - operating without speech recognition")
...
"""
    ...
    def _is_fortune_teller_info_true(self, targets: List[str], result: str) -> bool:
        ...
        actual_demon_count = len([p for p in target_players if self._is_demon(p.character)])
    ...
    async def _run_night_phase(self):
        ...
        # Announce night
        narration = await self.ai_storyteller.narrate("night_phase", {
        ...
        for character in wake_order:
            players_with_char = [p for p in self.game_state.players 
    ...
    async def _run_day_phase(self):
        ...
        if deaths:
            death_announcement = await self.ai_storyteller.narrate("death_announcement", {
                "deaths": deaths
            })
        else:
            death_announcement = await self.ai_storyteller.narrate("death_announcement", {
