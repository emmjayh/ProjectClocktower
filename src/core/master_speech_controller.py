...
from ..speech.voice_player_identification import VoicePlayerIdentifier, SecureVoiceActions
...
class MasterSpeechController(FullySpeechIntegratedController):
    ...
    async def _initialize_advanced_speech_systems(self, player_names: List[str]):
        ...
        if self.voice_id_enabled:
            ...
            self.voice_identifier = VoicePlayerIdentifier(self.speech_handler, player_names)
    ...
    async def run_game(self) -> Optional[Tuple[str, str]]:
        ...
        try:
            ...
            await self.speech_handler.speak("A mysterious error has occurred. The game ends.")
    ...
    async def _master_day_phase(self):
        ...
        if self.dramatic_storyteller:
            ...
            await self.speech_handler.speak(f"Day {self.engine.game_state.day_number} begins.")
        ...
        if executed_player:
            ...
            if executed and self.dramatic_storyteller:
                ...
                await self.speech_handler.speak(f"⚰️ {executed_player} has been executed!")
        else:
    ...
    async def _advanced_discussion_phase(self):
        ...
        while (asyncio.get_event_loop().time() - start_time) < discussion_time:
            ...
            command = await self.speech_handler.listen_for_command(
                keywords=["storyteller", "question", "nominate", "claim"],
                timeout=5.0
            )
        ...
        if interesting_commands and self.dramatic_storyteller:
            ...
            await self.speech_handler.speak("The discussion concludes. Time for nominations.")
            
    async def _process_discussion_command(self, command: str, interesting_commands: List[str]):
    ...
    async def _advanced_nomination_and_voting(self) -> Optional[str]:
        ...
        while (asyncio.get_event_loop().time() - start_time) < nomination_timeout:
            command = await self.speech_handler.listen_for_command(
                keywords=["nominate", "I nominate", "close nominations"],
                timeout=10.0
            )
    ...
    async def _process_advanced_nomination(self, command: str) -> Optional[Dict[str, str]]:
        ...
        if self.nlp_processor:
            ...
            if parsed.intent == Intent.NOMINATE and "nominee" in parsed.entities:
                ...
                if self.voice_id_enabled and self.secure_actions:
                    success, message, nominator = await self.secure_actions.secure_nomination(nominee)
    ...
    async def _advanced_voting(self, nomination: Dict[str, str]) -> Optional[str]:
        ...
        if self.dramatic_storyteller:
            await self.dramatic_storyteller.narrate_game_event("voting", nominee=nominee)
        else:
    ...
    async def _get_individual_vote_with_id(self, player) -> bool:
        ...
        if self.voice_id_enabled and self.secure_actions:
            ...
            success, message, voter = await self.secure_actions.secure_vote(True)  # Placeholder
            ...
            # Voice ID failed
            await self.speech_handler.speak(f"Could not verify {player.name}'s vote. Abstaining.")
            return False
        else:
    ...
    async def _master_night_phase(self):
        ...
        if self.dramatic_storyteller:
            ...
            await self.speech_handler.speak(f"🌙 Night {self.engine.game_state.day_number} begins.")
    ...
    async def _dramatic_victory_announcement(self, win_result: Tuple[str, str]):
        ...
        if self.dramatic_storyteller:
            ...
            await self.speech_handler.speak(f"🏆 {winning_team.upper()} TEAM WINS! {reason}")
        ...
        for player in self.engine.game_state.players:
            ...
            await self.speech_handler.speak(f"{player.name}: {player.character} ({status})")
    ...
    async def _announce_game_statistics(self):
        ...
        if total_commands > 0:
            success_rate = (self.command_stats["successful_commands"] / total_commands) * 100
        ...
        if self.voice_id_enabled:
            voice_attempts = self.command_stats["voice_id_successes"] + self.command_stats["voice_id_failures"]
            if voice_attempts > 0:
                voice_rate = (self.command_stats["voice_id_successes"] / voice_attempts) * 100
                await self.speech_handler.speak(
