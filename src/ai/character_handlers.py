...
class CharacterAbilityHandler:
    ...
    async def handle_fortune_teller(
        self, player: Player, targets: List[str], game_state: GameState
    ) -> str:
...
"""
        ...
        try:
            ...
            result = int(
                "".join(filter(str.isdigit, ai_response))[:1] or str(actual_pairs)
            )
        except Exception:
            result = actual_pairs
    ...
    async def handle_undertaker(
        self, player: Player, executed_player: Player, game_state: GameState
    ) -> str:
