...
from .autonomous_storyteller import AutonomousStoryteller
...
class FullyAutonomousGame:
    ...
    async def start_game(
        self, player_names: List[str], script: str = "trouble_brewing"
    ):
    ...
    def _create_game_state(self, player_names: List[str], script: str) -> GameState:
        ...
        for i, name in enumerate(player_names):
            player = Player(
                id=f"player_{i}", name=name, seat_position=i, status=PlayerStatus.ALIVE
            )
    ...
    def _get_character_distribution(self, player_count: int, script: str) -> List[str]:
        ...
        # Default fallback
        return (
            ["Fortune Teller", "Empath", "Washerwoman"]
            + ["Drunk"] * (player_count - 4)
            + ["Imp"]
        )
    ...
    async def _narrate_game_start(self):
        ...
        opening = await self.autonomous_storyteller.ai_storyteller.narrate(
            "game_start",
            {
    ...
    async def _simulate_first_night(self):
        ...
        await self.autonomous_storyteller._speak(
            "The first night begins. Everyone close your eyes."
        )
        ...
        # Fortune Teller
        ft_player = next(
            p for p in self.game_state.players if p.character == "Fortune Teller"
        )
        await self.autonomous_storyteller._speak(
            f"{ft_player.name}, Fortune Teller, wake up."
        )
        ...
        # Empath
        empath_player = next(
            p for p in self.game_state.players if p.character == "Empath"
        )
        await self.autonomous_storyteller._speak(
            f"{empath_player.name}, Empath, wake up."
        )
        ...
        await self.autonomous_storyteller._speak_privately(
            empath_player.name, empath_info
        )
