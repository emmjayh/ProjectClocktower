...
class GamePersistence:
    ...
    async def save_game(
        self, game_automation, filename: str = None, auto_save: bool = False
    ) -> str:
    ...
    async def restore_game_state(
        self, save_data: GameSaveData, game_automation
    ) -> bool:
        ...
        try:
            ...
            if hasattr(game_automation, "ability_system"):
                game_automation.ability_system.execution_history = (
                    ...
                    save_info.update(
                        {
                            ...
                            "player_count": len(
                                metadata.get("game_state", {}).get("players", [])
                            ),
                        }
                    )
    ...
    async def export_game_log(
        self, save_data: GameSaveData, filename: str = None
    ) -> str:
    ...
    async def _populate_save_data(self, save_data: GameSaveData, game_automation):
        ...
        save_data.current_phase = (
            game_automation.current_phase.name
            if game_automation.current_phase
            else None
        )
        save_data.phase_start_time = (
            game_automation.phase_start_time.isoformat()
            if game_automation.phase_start_time
            else None
        )
        ...
        if hasattr(game_automation, "live_monitor") and game_automation.live_monitor:
            save_data.live_monitoring_active = (
                game_automation.live_monitor.listening_active
            )

        # Ability executions
        if (
            hasattr(game_automation, "ability_system")
            and game_automation.ability_system
        ):
            save_data.ability_executions = [
        ...
        save_data.nomination_history = self._generate_nomination_history(
            game_automation
        )
...
class AutoSaveManager:
    ...
    async def start_auto_save(self, game_automation):
        ...
        self.auto_save_task = asyncio.create_task(self._auto_save_loop(game_automation))
    ...
    async def _auto_save_loop(self, game_automation):
        ...
        try:
            while True:
                ...
                if (
                    hasattr(game_automation, "is_running")
                    and game_automation.is_running
                ):
                    try:
