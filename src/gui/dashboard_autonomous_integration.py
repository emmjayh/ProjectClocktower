...
class EnhancedStorytellerDashboard:
    ...
    def _create_traditional_controls(self):
        ...
        # Platform selection
        ttk.Label(conn_frame, text="Platform:").grid(row=0, column=0, sticky="w", padx=5)
        ...
        # Room code
        ttk.Label(conn_frame, text="Room Code:").grid(row=1, column=0, sticky="w", padx=5)
        ...
        ttk.Button(phase_buttons, text="🌙 Night", command=self._manual_night).pack(side="left", padx=2)
        ttk.Button(phase_buttons, text="☀️ Day", command=self._manual_day).pack(side="left", padx=2)
        ttk.Button(phase_buttons, text="🗳️ Nominations", command=self._manual_nominations).pack(side="left", padx=2)
    ...
    def _switch_to_manual(self):
        ...
        if hasattr(self.autonomous_panel, 'is_running') and self.autonomous_panel.is_running:
    ...
    def _switch_to_autonomous(self):
        ...
        if not (hasattr(self.autonomous_panel, 'is_running') and self.autonomous_panel.is_running):
    ...
    def _switch_to_hybrid(self):
        ...
        self.mode_status.config(text="Hybrid mode - Both manual and AI control available")
