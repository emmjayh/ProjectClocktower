...
class GameEngine:
    ...
    def _select_characters(self, distribution: Dict[str, int]) -> List[str]:
        ...
        if distribution["townsfolk"] >= 4:
            ...
            if remaining_townsfolk > 0:
                available = [t for t in self.script.TOWNSFOLK if t not in core_townsfolk]
                selected.extend(random.sample(available, remaining_townsfolk))
        else:
            # For smaller games, pick random townsfolk
            selected.extend(random.sample(self.script.TOWNSFOLK, distribution["townsfolk"]))
        ...
        if distribution["outsiders"] > 0:
            ...
            if "Drunk" in self.script.OUTSIDERS:
                ...
                selected.extend(random.sample(self.script.OUTSIDERS, distribution["outsiders"]))
    ...
    def _assign_demon_bluffs(self):
        ...
        available_bluffs = [c for c in all_good_characters if c not in characters_in_play]
    ...
    def get_night_order(self, is_first_night: bool = False) -> List[str]:
        ...
        order = self.script.FIRST_NIGHT_ORDER if is_first_night else self.script.NIGHT_ORDER
        
        # Filter to only characters actually in play and alive
        characters_in_play = {p.character for p in self.game_state.players 
    ...
    def check_win_condition(self) -> Optional[Tuple[str, str]]:
        ...
        for player in self.game_state.players:
            if (player.character == "Saint" and 
                player.status == PlayerStatus.DEAD and 
                hasattr(player, 'execution_day') and 
                player.execution_day is not None):
