...
class EnhancedStoryteller:
    ...
    async def narrate_dawn(self, deaths: List[str]) -> None:
        ...
        if not deaths:
            ...
            if len(deaths) == 1:
                ...
                death_text = f"The bodies of {death_list} are found, victims of the darkness."
    ...
    async def deliver_private_information(self, player_name: str, info: str, character: str) -> None:
    ...
    async def announce_vote_results(self, yes_votes: int, no_votes: int, threshold: int, 
                                   execute: bool, nominee: str) -> None:
        """Announce vote results with drama"""
        
        result_text = f"The votes are tallied: {yes_votes} for execution, {no_votes} against."
    ...
    async def _speak_with_style(self, text: str, style: NarrativeStyle) -> None:
        ...
        if style in [NarrativeStyle.DRAMATIC, NarrativeStyle.MYSTERIOUS, NarrativeStyle.OMINOUS]:
...
class GameStateAnnouncer:
    ...
    async def announce_phase_change(self, old_phase: str, new_phase: str, day_number: int):
    ...
    async def announce_ability_activation(self, character: str, player_name: str):
        ...
        if character in ability_announcements:
            ...
            await self.storyteller._speak_with_style(announcement, NarrativeStyle.DRAMATIC)
    ...
    async def narrate_game_event(self, event_type: str, **kwargs):
        ...
        if event_type == "game_start":
            ...
            await self.enhanced_storyteller.announce_vote_results(
                kwargs["yes_votes"], kwargs["no_votes"], kwargs["threshold"],
                kwargs["execute"], kwargs["nominee"]
            )
