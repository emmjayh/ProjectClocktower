...
class ProjectSetup:
    ...
    async def setup_everything(self):
        ...
        try:
            ...
            logger.info("✅ Setup complete! Ready to run Blood on the Clocktower games.")
    ...
    async def _install_piper(self):
        ...
        try:
            # Download
            subprocess.run([
                "wget", "-O", str(piper_dir / "piper.tar.gz"), url
            ], check=True)
    ...
    async def test_installation(self):
        ...
        try:
            ...
            model = whisper.load_model("base", download_root=str(self.models_dir / "whisper_base"))
