...
class BotCAppAdapter:
    ...
    async def _try_observer_mode(self, room_code: str) -> bool:
        ...
        try:
            # Connect as observer
            import websockets
            observer_url = f"{self.api.base_url.replace('http', 'ws')}/observe/{room_code}"
    ...
    async def _enhance_state_data(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "players" in enhanced:
                enhanced["player_analysis"] = await self._analyze_players(enhanced["players"])
    ...
    async def _get_night_order(self) -> List[str]:
        ...
        return [
            ...
            "Ravenkeeper", "Empath", "Fortune Teller", 
            "Undertaker", "Butler"
        ]
    ...
    async def send_enhanced_action(self, action_type: str, **kwargs) -> bool:
        ...
        try:
            ...
            success = await self.api.send_storyteller_action(action_type, processed_data)
            
            # If websocket fails, try HTTP fallback
            if not success and hasattr(self.api, 'connection_mode'):
                success = await self.api._send_botc_http_action(action_type, processed_data)
    ...
    async def _preprocess_action(self, action_type: str, data: Dict[str, Any]) -> Dict[str, Any]:
