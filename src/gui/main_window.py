...
class MainWindow:
    ...
    def _create_setup_tab(self):
        ...
        # Player names input
        ttk.Label(self.setup_frame, text="Player Names (one per line):", style='Subtitle.TLabel').pack(pady=10)
        ...
        ttk.Label(script_frame, text="Script:", style='Subtitle.TLabel').pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(voice_frame, text="Storyteller Voice:", style='Subtitle.TLabel').pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(file_frame, text="💾 Save Setup", command=self._save_setup).pack(side=tk.LEFT, padx=5)
        ttk.Button(file_frame, text="📂 Load Setup", command=self._load_setup).pack(side=tk.LEFT, padx=5)
    ...
    def _create_game_tab(self):
        ...
        ttk.Label(status_frame, text="Game Status:", style='Subtitle.TLabel').pack(anchor=tk.W)
        ...
        ttk.Label(phase_frame, text="Phase Control:", style='Subtitle.TLabel').pack(anchor=tk.W)
        
        phase_buttons = ttk.Frame(phase_frame, style='Dark.TFrame')
        phase_buttons.pack(anchor=tk.W, pady=5)
        
        self.night_button = ttk.Button(phase_buttons, text="🌙 Start Night", command=self._start_night)
        self.night_button.pack(side=tk.LEFT, padx=5)
        
        self.day_button = ttk.Button(phase_buttons, text="☀️ Start Day", command=self._start_day)
        self.day_button.pack(side=tk.LEFT, padx=5)
        
        self.nomination_button = ttk.Button(phase_buttons, text="🗳️ Open Nominations", command=self._open_nominations)
        ...
        ttk.Label(speech_frame, text="Speech Control:", style='Subtitle.TLabel').pack(anchor=tk.W)
        
        speech_buttons = ttk.Frame(speech_frame, style='Dark.TFrame')
        speech_buttons.pack(anchor=tk.W, pady=5)
        
        self.listen_button = ttk.Button(speech_buttons, text="🎤 Listen", command=self._toggle_listening)
        self.listen_button.pack(side=tk.LEFT, padx=5)
        
        self.speak_button = ttk.Button(speech_buttons, text="🔊 Speak", command=self._manual_speak)
        self.speak_button.pack(side=tk.LEFT, padx=5)
        
        # Manual input for speech
        ttk.Label(self.game_frame, text="Manual Announcement:", style='Subtitle.TLabel').pack(anchor=tk.W, pady=(10,0))
        ...
        # Game log
        ttk.Label(self.game_frame, text="Game Log:", style='Subtitle.TLabel').pack(anchor=tk.W, pady=(10,0))
    ...
    def _create_settings_tab(self):
        ...
        # Whisper model
        ttk.Label(speech_settings, text="Whisper Model:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        # Voice threshold
        ttk.Label(speech_settings, text="Voice Threshold:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        # Story complexity
        ttk.Label(ai_settings, text="Story Complexity:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        auto_check = ttk.Checkbutton(
            ai_settings,
            text="Auto-manage game phases",
            variable=self.auto_mode_var
        )
        auto_check.grid(row=1, column=0, columnspan=2, sticky=tk.W, padx=5, pady=5)
        
        # Save settings button
        ttk.Button(self.settings_frame, text="💾 Save Settings", command=self._save_settings).pack(pady=20)
    ...
    def _start_game(self):
        ...
        try:
            ...
            if len(player_names) < 5:
                messagebox.showerror("Error", "Need at least 5 players to start a game!")
                return
            ...
            asyncio.run_coroutine_threadsafe(
                self._async_start_game(player_names), 
                self.event_loop
            )
    ...
    def _start_night(self):
        ...
        if self.storyteller and self.is_running:
            asyncio.run_coroutine_threadsafe(
                self.storyteller._transition_to_night(),
                self.event_loop
            )
    ...
    def _start_day(self):
        ...
        if self.storyteller and self.is_running:
            asyncio.run_coroutine_threadsafe(
                self.storyteller._transition_to_day(),
                self.event_loop
            )
    ...
    def _open_nominations(self):
        ...
        if self.storyteller and self.is_running:
            asyncio.run_coroutine_threadsafe(
                self.storyteller._handle_nominations(),
                self.event_loop
            )
    ...
    def on_closing(self):
        ...
        if self.is_running:
            if messagebox.askokcancel("Quit", "Game is running. Are you sure you want to quit?"):
                self._stop_game()
                self.root.destroy()
        else:
            self.root.destroy()
