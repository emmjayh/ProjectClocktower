...
class CharacterAbility(ABC):
    ...
    @abstractmethod
    async def execute(self, player: Player, game_state: GameState, 
                     targets: List[str] = None, **kwargs) -> AbilityExecution:
        """Execute the ability"""
        pass
...
class ScarletWomanAbility(CharacterAbility):
    ...
    async def execute(self, player: Player, game_state: GameState, 
        ...
        imp_alive = any(p.character == "Imp" and p.is_alive() 
...
class WasherwomanAbility(CharacterAbility):
    ...
    def _generate_true_info(self, game_state: GameState) -> str:
        ...
        townsfolk = [p for p in game_state.players if p.team == "good" and p.character != "Washerwoman"]
    ...
    def _generate_false_info(self, game_state: GameState) -> str:
        ...
        fake_character = random.choice(["Librarian", "Investigator", "Chef", "Empath", "Fortune Teller"])
...
class ButlerAbility(CharacterAbility):
    ...
    def check_voting_restriction(self, player: Player, vote_target: str, 
...
class VirginAbility(CharacterAbility):
    ...
    async def execute(self, player: Player, game_state: GameState, 
        ...
        if virgin_active and nominator.team == "good" and nominator.character != "Virgin":
            ...
            self.logger.info(f"Virgin power: {nominator_name} dies for nominating Virgin")
        else:
...
class AbilitySystem:
    ...
    async def execute_ability(self, character: str, player: Player, 
                            game_state: GameState, trigger: TriggerType,
                            targets: List[str] = None, **kwargs) -> Optional[AbilityExecution]:
    ...
    def get_execution_history(self, character: str = None, limit: int = 10) -> List[AbilityExecution]:
