...
from .speech_integrated_controller import SpeechIntegratedController, SpeechPlayerChoices
...
class EnhancedNightSpeechInterface:
    ...
    def __init__(self, speech_handler: SpeechHandler, ability_dispatcher: AbilityDispatcher):
        ...
        self.wake_phrases = {
            ...
            "Chef": [
                "Chef, wake up.", 
                "You learn about evil player pairs."
            ],
            "Poisoner": [
                "Poisoner, wake up.",
    ...
    async def handle_enhanced_night_phase(self, game_controller: SpeechIntegratedController):
        ...
        # Get night order
        is_first_night = game_controller.engine.game_state.phase == GamePhase.FIRST_NIGHT
        ...
        for character in night_order:
            await self._wake_character_enhanced(character, game_controller, is_first_night)
            
        await self.speech_handler.speak("🌅 Dawn breaks. Everyone may open their eyes.")
        
    async def _wake_character_enhanced(self, character: str, game_controller: SpeechIntegratedController, is_first_night: bool):
        """Wake character with enhanced speech interaction"""
        
        # Find players with this character
        players_with_char = [
            p for p in game_controller.engine.game_state.players 
        ...
        await self._handle_enhanced_character_action(player, game_controller, is_first_night)
    ...
    async def _handle_enhanced_character_action(self, player: Player, game_controller: SpeechIntegratedController, is_first_night: bool):
        ...
        if character == "Fortune Teller":
            ...
            await self.speech_handler.speak_to_player(player.name, "You have no action tonight.")
            await asyncio.sleep(2)
            
    async def _handle_enhanced_fortune_teller(self, player: Player, game_controller: SpeechIntegratedController):
        """Enhanced Fortune Teller interaction"""
        
        valid_targets = [p.name for p in game_controller.engine.get_alive_players() if p.name != player.name]
        
        if len(valid_targets) < 2:
            await self.speech_handler.speak_to_player(player.name, "Not enough players to choose from.")
            return
            
        # Enhanced prompting
        await self.speech_handler.speak_to_player(
            player.name, 
            "Choose two players. You will learn if either is the demon."
        )
    ...
    async def _handle_enhanced_empath(self, player: Player, game_controller: SpeechIntegratedController):
        """Enhanced Empath interaction"""
        
        await self.speech_handler.speak_to_player(
            player.name,
            "You sense the evil in your immediate neighbors."
        )
        
        await asyncio.sleep(2)
        
        # Execute ability
        ability_result = game_controller.ability_dispatcher.execute_ability("Empath", player)
    ...
    async def _handle_enhanced_washerwoman(self, player: Player, game_controller: SpeechIntegratedController):
        """Enhanced Washerwoman interaction"""
        
        await self.speech_handler.speak_to_player(
            player.name,
            "You investigate the town's residents..."
        )
        
        await asyncio.sleep(2)
        
        ability_result = game_controller.ability_dispatcher.execute_ability("Washerwoman", player)
        
        # Enhanced information delivery
        info_parts = ability_result.information.split(": ")[1]
        await self.speech_handler.speak_to_player(player.name, f"You discover: {info_parts}")
        
        game_controller.ability_results.append(ability_result)
        
    async def _handle_enhanced_chef(self, player: Player, game_controller: SpeechIntegratedController):
        """Enhanced Chef interaction"""
        
        await self.speech_handler.speak_to_player(
            player.name,
            "You observe who sits together at meals..."
        )
        
        await asyncio.sleep(2)
        
        ability_result = game_controller.ability_dispatcher.execute_ability("Chef", player)
    ...
    async def _handle_enhanced_poisoner(self, player: Player, game_controller: SpeechIntegratedController):
        ...
        await self.speech_handler.speak_to_player(
            player.name,
            "Choose someone to poison. Their abilities will malfunction."
        )
    ...
    async def _handle_enhanced_imp(self, player: Player, game_controller: SpeechIntegratedController):
        ...
        if choice_result.is_valid:
            ...
            if target == player.name:
                ...
                await self.speech_handler.speak_to_player(
                    player.name, f"You mark {target} for death. They will not see tomorrow's dawn."
                )
...
class VoiceControlledVoting:
    ...
    async def conduct_enhanced_vote(self, nominee: str, alive_players: List[Player]) -> Tuple[bool, int, int]:
        ...
        await self._announce_vote_results(yes_votes, no_votes, threshold, execute, nominee)
        
        return execute, yes_votes, no_votes
        
    async def _individual_vote(self, player: Player, yes_count: int, no_count: int) -> bool:
        ...
        for attempt in range(3):
            ...
            if command:
                ...
                await self.speech_handler.speak(f"{player.name}, please state your vote.")
                
        # Default to no vote if no clear response
        await self.speech_handler.speak(f"{player.name} abstains.")
        return False
        
    async def _announce_vote_results(self, yes_votes: int, no_votes: int, threshold: int, execute: bool, nominee: str):
        ...
        await self.speech_handler.speak(f"The threshold for execution is {threshold} votes.")
...
class FullySpeechIntegratedController(SpeechIntegratedController):
    ...
    async def _conduct_speech_vote(self, nominee: str) -> Optional[str]:
        ...
        if self.voice_voting:
            ...
            execute, yes_votes, no_votes = await self.voice_voting.conduct_enhanced_vote(
                nominee, alive_players
            )
