...
class AIStoryteller:
    ...
    async def _setup_game(self) -> None:
        ...
        intro_text = await self.narrator.generate_opening_story(
            len(self.game_state.players), self.game_state.script_name
        )
    ...
    async def _distribute_characters(self) -> None:
        ...
        characters = self.character_data.get_distribution(
            player_count, self.game_state.script_name
        )
        ...
        await self.speech_handler.speak(
            "Characters have been distributed. Check your tokens quietly."
        )
    ...
    async def _wake_minions(self) -> None:
        ...
        minions = [
            ...
            if self.character_data.is_minion(p.character)
        ]
        demon = next(
            p
            for p in self.game_state.players
            if self.character_data.is_demon(p.character)
        )

        if minions:
            minion_names = [p.name for p in minions]
            await self.speech_handler.speak(
                f"Minions: {', '.join(minion_names)}, wake up."
            )
    ...
    async def _wake_demon(self) -> None:
        ...
        if minions:
            minion_names = [p.name for p in minions]
            await self.speech_handler.speak(
                f"These are your minions: {', '.join(minion_names)}"
            )

        bluff_text = (
            f"Your bluff characters are: {', '.join(self.game_state.demon_bluffs)}"
        )
    ...
    async def _process_first_night_abilities(self) -> None:
        ...
        for character_name in night_order:
            players_with_character = [
                p for p in self.game_state.players if p.character == character_name
            ]

            for player in players_with_character:
                if not player.is_drunk and not player.is_poisoned:
                    await self._execute_night_ability(
                        player, character_name, is_first_night=True
                    )

    async def _execute_night_ability(
        self, player: Player, character: str, is_first_night: bool = False
    ) -> None:
    ...
    async def _transition_to_day(self) -> None:
        ...
        if deaths:
            death_text = await self.narrator.generate_death_announcement(
                deaths, self.game_state.day_number
            )
    ...
    async def _conduct_vote(self, nomination: Dict) -> None:
        ...
        await self.speech_handler.speak(
            f"Voting on {nominee}. Raise your hand to vote."
        )
        ...
        required_votes = (
            len([p for p in self.game_state.players if p.status == PlayerStatus.ALIVE])
            // 2
            + 1
        )

        await self.speech_handler.speak(
            f"{vote_count} votes. {required_votes} required."
        )
    ...
    async def _execute_player(self, player_name: str) -> None:
        ...
        execution_text = await self.narrator.generate_execution_story(
    ...
    async def _check_win_conditions(self) -> bool:
        ...
        alive_players = [
            p for p in self.game_state.players if p.status == PlayerStatus.ALIVE
        ]
    ...
    async def _end_game(self, winner: str, reason: str) -> None:
        ...
        ending_story = await self.narrator.generate_ending_story(
            winner, reason, self.game_state
        )
...
class RuleEnforcer:
    ...
    def validate_nomination(self, nomination: Dict, game_state: GameState) -> bool:
        ...
        nominator_player = next(
            (p for p in game_state.players if p.name == nominator), None
        )
        ...
        nominee_player = next(
            (p for p in game_state.players if p.name == nominee), None
        )
...
class NarrativeAI:
    ...
    async def generate_execution_story(
    ...
    async def generate_ending_story(
        self, winner: str, reason: str, game_state: GameState
    ) -> str:
...
class GameBalancer:
    ...
    async def modify_information_for_drunk_poisoned(
        self, info: str, player: Player, game_state: GameState
    ) -> str:
