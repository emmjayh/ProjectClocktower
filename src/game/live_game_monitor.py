...
class LiveGameMonitor:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            config = ListenerConfig(
                ...
                wake_words=[
                    "nominate", "i nominate", "vote", "execution",
                    "storyteller", "demon", "minion", "outsider", 
                    "townsfolk", "clocktower", "close nominations"
                ],
            ...
            self.listener.register_command_callback("nomination", self._handle_nomination)
            self.listener.register_command_callback("vote", self._handle_vote)
            self.listener.register_command_callback("general", self._handle_general_command)
            self.listener.register_command_callback("character_mention", self._handle_character_mention)
    ...
    async def _handle_nomination(self, command: Dict[str, Any]):
        ...
        try:
            if not self.nominations_open:
                self.logger.info(f"Nomination ignored - nominations not open (phase: {self.current_phase})")
                return
            ...
            if validation["valid"]:
                ...
                self.logger.info(f"✅ Valid nomination: {nominator} nominates {nominee}")
                
                # Trigger callbacks
                await self._trigger_event("nomination_received", {
                    "nominator": nominator,
                    "nominee": nominee,
                ...
                await self._trigger_event("nomination_invalid", {
                    "nominator": nominator,
                    "nominee": nominee,
    ...
    async def _handle_general_command(self, command: Dict[str, Any]):
        ...
        if any(phrase in text for phrase in ["close nominations", "end nominations", "voting time"]):
        ...
        elif any(phrase in text for phrase in ["start voting", "time to vote", "begin vote"]):
    ...
    async def _handle_character_mention(self, command: Dict[str, Any]):
        ...
        await self._trigger_event("character_mention", {
            "text": text,
            "characters": wake_words,
            "timestamp": datetime.now()
        })
    ...
    async def _announce_nomination(self, nomination: NominationEvent):
        ...
        await self._trigger_event("tts_announce", {
            "message": message,
            "priority": "high"
        })
    ...
    async def _trigger_clarification(self, event_type: str, command: Dict[str, Any]):
        ...
        await self._trigger_event("request_clarification", {
            "type": event_type,
            "command": command,
    ...
    def start_voting_on_nomination(self, nomination_index: int = -1):
        ...
        self.logger.info(f"🗳️ Started voting on: {nomination.nominator} nominates {nomination.nominee}")
        return True
    ...
    def end_voting(self) -> Dict[str, Any]:
        ...
        self.logger.info(f"🗳️ Voting ended: {results['yes_votes']} yes, {results['no_votes']} no")
