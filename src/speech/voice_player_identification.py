...
class VoiceFeatureExtractor:
    ...
    def extract_features(
        ...
        try:
            ...
            features.extend([np.mean(mel_features), np.std(mel_features)])
    ...
    def _extract_mel_features(
        self, magnitude: np.ndarray, sample_rate: int
    ) -> np.ndarray:
...
class VoicePlayerIdentifier:
    ...
    async def _train_player_voice(self, player_name: str) -> bool:
        ...
        try:
            ...
            for attempt in range(3):
                await self.speech_handler.speak(
                    f"Recording {attempt + 1}. Say your name now."
                )
            ...
            if len(voice_samples) >= self.min_training_samples:
                ...
                await self.speech_handler.speak(
                    f"Voice profile created for {player_name}."
                )
                return True
            else:
                await self.speech_handler.speak(
                    f"Failed to get enough voice samples for {player_name}."
                )
                return False
    ...
    async def identify_speaker(
        self, audio_data: bytes = None, timeout: float = 3.0
    ...
    def _calculate_voice_similarity(
        self, features: np.ndarray, profile: VoiceProfile
    ) -> float:
    ...
    async def verify_player_identity(
        self, claimed_name: str, max_attempts: int = 3
    ) -> bool:
        ...
        for attempt in range(max_attempts):
            ...
            if (
                identified_name == claimed_name
                and confidence > self.identification_threshold
            ):
    ...
    async def secure_command_with_voice_id(
        ...
        if identified_player and confidence > self.identification_threshold:
            ...
            await self.speech_handler.speak(
                "Could not identify speaker. Command ignored."
            )
    ...
    def get_identification_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "min_samples": (
                min(p.training_samples for p in self.voice_profiles.values())
                if self.voice_profiles
                else 0
            ),
            "max_samples": (
                max(p.training_samples for p in self.voice_profiles.values())
                if self.voice_profiles
                else 0
            ),
            "avg_samples": (
                np.mean([p.training_samples for p in self.voice_profiles.values()])
                if self.voice_profiles
                else 0
            ),
...
class SecureVoiceActions:
    ...
    def __init__(self, voice_identifier: VoicePlayerIdentifier):
        ...
        self.secure_actions = {"nominate", "vote", "claim", "ability_target"}

    async def process_secure_command(
        self, action_type: str, command_data: Dict[str, Any]
        ...
        if confidence < self.voice_identifier.identification_threshold:
            return (
                False,
                f"Voice identification confidence too low: {confidence:.2f}",
                command_data,
            )
