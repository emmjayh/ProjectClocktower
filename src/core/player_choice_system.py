...
class PlayerChoiceSystem:
    ...
    def request_choice(self, request: ChoiceRequest, 
                      callback: Optional[Callable[[ChoiceResult], None]] = None) -> str:
        """Request a choice from a player"""
        
        choice_id = f"{request.player_name}_{request.character}_{len(self.pending_choices)}"
    ...
    def _validate_choice(self, request: ChoiceRequest, choice_data: Dict[str, Any]) -> ChoiceResult:
        ...
        if request.choice_type == ChoiceType.SINGLE_TARGET:
            ...
            elif target1 not in request.valid_targets or target2 not in request.valid_targets:
                result.is_valid = False
...
class MVPChoiceTemplates:
    ...
    @staticmethod
    def fortune_teller_choice(player: Player, valid_targets: List[str]) -> ChoiceRequest:
...
class MockPlayerChoices:
    ...
    async def request_choice_async(self, request: ChoiceRequest) -> ChoiceResult:
        ...
        elif request.choice_type == ChoiceType.DUAL_TARGET and len(request.valid_targets) >= 2:
            return ChoiceResult(
                player_name=request.player_name,
                character=request.character,
                choice_type=request.choice_type,
...
if __name__ == "__main__":
    ...
    async def test_choices():
        ...
        ft_request = MVPChoiceTemplates.fortune_teller_choice(
            Player("1", "Alice", 0, "Fortune Teller"),
            players[1:]  # Can't target self
        )
        ...
        poisoner_request = MVPChoiceTemplates.poisoner_choice(
            Player("2", "Frank", 5, "Poisoner"),
            players  # Can target anyone
        )
