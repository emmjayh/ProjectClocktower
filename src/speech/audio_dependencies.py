...
def check_continuous_listening_support() -> Dict[str, bool]:
    ...
    result["features"]["continuous_listening"] = result["supported"] and DEPENDENCIES["pyaudio"]
    result["features"]["speaker_identification"] = result["supported"] and DEPENDENCIES["librosa"] and DEPENDENCIES["sklearn"]
    result["features"]["enhanced_tts"] = DEPENDENCIES["pygame"] and DEPENDENCIES["requests"]
...
def log_audio_capabilities():
    ...
    if support["supported"]:
        ...
        if support["features"]["speaker_identification"]:
            ...
            logger.warning("⚠️ Speaker identification disabled (missing librosa/sklearn)")
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            if not DEPENDENCIES.get(name, False):
                ...
                raise ImportError(f"{feature} requires {name} - install with: pip install {name}")
...
__all__ = [
    ...
    'require_dependency',
    'np', 'pyaudio', 'whisper', 'librosa', 'pygame', 'requests',
