...
class GameNarrator:
    ...
    async def narrate_game_start(self, player_count: int, script: str) -> str:
        ...
        if self.use_ai:
            try:
                return await self.ai_client.narrate("game_start", {
    ...
    async def narrate_night_start(self, night_number: int, alive_count: int) -> str:
        ...
        if self.use_ai:
            try:
                return await self.ai_client.narrate("night_phase", {
                    "night": night_number,
                    "alive": alive_count
                })
            except Exception as e:
    ...
    async def announce_deaths(self, deaths: List[str]) -> str:
        ...
        if self.use_ai:
            try:
                return await self.ai_client.narrate("death_announcement", {
                    "deaths": deaths
                })
            except Exception as e:
                self.logger.error(f"AI narration failed: {e}")
    ...
    async def narrate_nomination(self, nominator: str, nominee: str) -> str:
        ...
        if self.use_ai:
            try:
                return await self.ai_client.narrate("nomination", {
                    "nominator": nominator,
                    "nominee": nominee
                })
            except Exception as e:
    ...
    async def narrate_execution(self, player_name: str, character: Optional[str] = None) -> str:
        ...
        if self.use_ai:
            try:
                return await self.ai_client.narrate("execution", {
                    "player": player_name,
                    "character": character
                })
            except Exception as e:
    ...
    async def announce_victory(self, winning_team: str, reason: str) -> str:
        ...
        if self.use_ai:
            try:
                return await self.ai_client.narrate("victory", {
                    "team": winning_team,
                    "reason": reason
                })
            except Exception as e:
