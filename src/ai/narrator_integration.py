...
class AIGameNarrator:
    ...
    async def initialize(self) -> bool:
        ...
        if self.use_ai and self.ai_storyteller:
            ...
            if not success:
                self.logger.warning("Failed to initialize AI - using fallback narration")
                self.use_ai = False
            return success
        return True
    
    async def narrate_game_start(self, player_count: int, script: str = "Trouble Brewing") -> str:
    ...
    async def narrate_night_phase(self, night_number: int, alive_count: int) -> str:
        ...
        # Fallback
        if night_number == 1:
            return "The first night descends upon the town. Everyone, close your eyes..."
        else:
    ...
    async def narrate_execution(self, player_name: str, character: Optional[str] = None) -> str:
    ...
    async def announce_victory(self, winning_team: str, reason: str) -> str:
        ...
        if winning_team.lower() == "good":
            return f"Light banishes darkness! The forces of good have triumphed. {reason}"
        else:
    ...
    async def describe_character(self, character: str) -> str:
        ...
        if self.use_ai and self.ai_storyteller:
            ...
            return await self.ai_storyteller.narrate("character_description", {"question": instruction})
        ...
        return descriptions.get(character, f"The {character} has a unique ability in this game.")
