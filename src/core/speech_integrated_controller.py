...
from .player_choice_system import (
    ChoiceRequest, ChoiceResult, ChoiceType, MVPChoiceTemplates
)
...
class SpeechPlayerChoices:
    ...
    def __init__(self, speech_handler: SpeechHandler, command_parser: SpeechCommandParser):
    ...
    async def request_choice_async(self, request: ChoiceRequest) -> ChoiceResult:
        ...
        self.logger.info(f"Requesting speech choice from {request.player_name} ({request.character})")
        ...
        for attempt in range(3):
            ...
            if command:
                ...
                if action and action["type"] == "dual_target":
                    ...
                    if (target1 in request.valid_targets and 
                        target2 in request.valid_targets and 
                        target1 != target2):
                        await self.speech_handler.speak_to_player(
                    ...
                    await self.speech_handler.speak_to_player(
                        request.player_name, "Please say two names with 'and' between them."
                    )
            else:
                await self.speech_handler.speak_to_player(
                    request.player_name, "No response heard. Try again."
...
class SpeechIntegratedController(MVPGameController):
    ...
    async def setup_game(self, player_names: List[str]) -> bool:
        ...
        try:
            ...
            self.speech_choices = SpeechPlayerChoices(self.speech_handler, self.command_parser)
    ...
    async def _run_day_phase(self):
        ...
        if executed_player:
            ...
            await self.speech_handler.speak("No execution today. The town remains divided.")
        ...
        while (asyncio.get_event_loop().time() - start_time) < nomination_timeout:
            ...
            if command:
                ...
                if nomination and nomination["nominee"]:
                    ...
                    await self.speech_handler.speak("I didn't understand that nomination.")
