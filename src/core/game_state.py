...
class Player:
    ...
    def add_reminder(
        self, token_type: str, description: str, source: Optional[str] = None
    ):
        """Add a reminder token to this player"""
        self.reminder_tokens.append(
            ReminderToken(token_type, description, True, source)
        )
...
class GameStateManager:
    ...
    def load_game_state(self, game_id: str) -> Optional[GameState]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    "SELECT game_data FROM games WHERE game_id = ?", (game_id,)
                )
    ...
    def _serialize_game_state(self, game_state: GameState) -> str:
        ...
        for nomination in data["nominations"]:
            nomination["timestamp"] = (
                nomination["timestamp"].isoformat()
                if isinstance(nomination["timestamp"], datetime)
                else nomination["timestamp"]
            )
    ...
    def create_backup(self, game_id: str) -> bool:
        ...
        try:
            ...
            if game_state:
                backup_id = (
                    f"{game_id}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                )
    ...
    def list_games(self) -> List[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                return [
                    {"game_id": row[0], "created_at": row[1], "updated_at": row[2]}
...
class GameStateValidator:
    ...
    def validate_nomination(
        nomination: Nomination, game_state: GameState
    ) -> tuple[bool, str]:
