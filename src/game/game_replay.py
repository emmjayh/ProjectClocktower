...
class GameRecorder:
    ...
    def record_event(self, event_type: EventType, player: str = None, 
                    target: str = None, data: Dict[str, Any] = None,
                    description: str = "", automated: bool = True) -> GameEvent:
    ...
    def record_nomination(self, nominator: str, nominee: str, 
                         successful: bool = True, reason: str = ""):
        ...
        return self.record_event(
            ...
            description=f"{nominator} nominates {nominee}" + (f" ({reason})" if reason else "")
        )
    ...
    def record_vote(self, voter: str, vote: str, nomination_target: str = None):
        ...
        return self.record_event(
            ...
            description=f"{voter} votes {vote}" + (f" on {nomination_target}" if nomination_target else "")
        )
        
    def record_execution(self, executed_player: str, vote_count: int, 
                        threshold: int, cause: str = "voting"):
    ...
    def record_death(self, player: str, cause: str, killer: str = None):
        ...
        return self.record_event(
            ...
            description=f"{player} dies" + (f" (killed by {killer})" if killer else f" ({cause})")
        )
        
    def record_ability_execution(self, player: str, character: str, 
        ...
        return self.record_event(
            ...
            description=f"{player} ({character}) uses ability" + (f" on {targets}" if targets else "")
        )
    ...
    def record_announcement(self, message: str, speaker: str = "Storyteller"):
        ...
        return self.record_event(
            ...
            description=f"{speaker}: {message[:50]}..." if len(message) > 50 else f"{speaker}: {message}"
        )
    ...
    def record_game_end(self, winner: str, reason: str = ""):
        ...
        return self.record_event(
            ...
            data={
                "winner": winner,
                "reason": reason,
                "duration": (self.metadata.game_end_time - self.metadata.game_start_time).total_seconds()
            },
            description=f"Game ended - {winner} wins" + (f" ({reason})" if reason else "")
        )
        
    def get_events(self, event_type: EventType = None, 
                  player: str = None, limit: int = None) -> List[GameEvent]:
        """Get filtered events"""
        ...
        if player:
            filtered_events = [e for e in filtered_events if e.player == player or e.target == player]
...
class GamePlayer:
        ...
        try:
            while self.current_position < len(self.events) and self.is_playing:
                ...
                if self.auto_advance and self.current_position < len(self.events):
                    ...
                    delay = (next_event.timestamp - event.timestamp) / self.playback_speed
    ...
    def get_progress(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "progress_percent": (self.current_position / len(self.events)) * 100 if self.events else 0,
            "is_playing": self.is_playing,
            "speed": self.playback_speed,
            "current_event": self.events[self.current_position].to_dict() if self.current_position < len(self.events) else None
        }
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "event_counts": {
                event_type.name: len([e for e in self.events if e.event_type == event_type])
    ...
    def _calculate_player_stats(self) -> Dict[str, Dict[str, int]]:
        ...
        for player in self.metadata.players:
            stats[player] = {
                "nominations_made": len([e for e in self.events if e.event_type == EventType.NOMINATION and e.player == player]),
                "votes_cast": len([e for e in self.events if e.event_type == EventType.VOTE and e.player == player]),
                "times_nominated": len([e for e in self.events if e.event_type == EventType.NOMINATION and e.target == player]),
                "ability_uses": len([e for e in self.events if e.event_type == EventType.ABILITY_EXECUTION and e.player == player]),
                "died": len([e for e in self.events if e.event_type == EventType.DEATH and e.target == player]) > 0,
                "executed": len([e for e in self.events if e.event_type == EventType.EXECUTION and e.target == player]) > 0
            }
...
class ReplayManager:
    ...
    async def load_replay(self, filename: str) -> GamePlayer:
        ...
        try:
            ...
            events = [GameEvent.from_dict(event_data) for event_data in replay_data["events"]]
