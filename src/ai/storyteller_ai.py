...
from ..core.game_state import GameState, Player, PlayerStatus, Team, GamePhase
...
class StorytellerAI:
    ...
    async def generate_opening_story(self, script: str, player_count: int) -> str:
        ...
        self._log_decision("opening_story", 1.0, f"Generated opening for {script}", {"story": story})
        
        return story
        
    def decide_fortune_teller_result(self, fortune_teller: Player, target1: str, target2: str, game_state: GameState) -> bool:
        ...
        actual_result = (self._is_demon(player1.character) or self._is_demon(player2.character))
        ...
        if fortune_teller.is_drunk or fortune_teller.is_poisoned:
            ...
            if self.balance_preference == "favor_evil" or random.random() < 0.3:
                ...
                reasoning = "True info given despite drunk/poisoned (storyteller choice)"
        else:
            result = actual_result
        ...
        self._log_decision("fortune_teller", 0.8, reasoning, {
    ...
    def decide_empath_result(self, empath: Player, left_neighbor: Player, right_neighbor: Player, game_state: GameState) -> int:
        ...
        self._log_decision("empath", 0.9, reasoning, {
            "empath": empath.name,
    ...
    def decide_chef_result(self, chef: Player, game_state: GameState) -> int:
        ...
        self._log_decision("chef", 0.9, reasoning, {
            "chef": chef.name,
            "result": result,
            "actual": actual_count
        })
    ...
    def decide_investigator_result(self, investigator: Player, game_state: GameState) -> Tuple[str, str, str]:
        ...
        if investigator.is_drunk or investigator.is_poisoned:
            ...
            non_minions = [p for p in game_state.players if not self._is_minion(p.character)]
            ...
            if minions:
                ...
                non_minions = [p for p in game_state.players if p.name != minion.name and not self._is_minion(p.character)]
                if non_minions:
                    ...
                    targets = random.sample(game_state.players, 2)
                    character = targets[0].character if self._is_minion(targets[0].character) else targets[1].character
        ...
        self._log_decision("investigator", 0.8, reasoning, {
    ...
    def decide_demon_kill(self, demon: Player, game_state: GameState) -> Optional[str]:
        ...
        possible_targets = [
            ...
            if p.is_alive() and p.name != demon.name and not self._is_minion(p.character)
        ]
        ...
        for target in possible_targets:
            ...
            if self.character_data.get(target.character, {}).get("ability") == "ongoing_info":
                score += 3
            elif self.character_data.get(target.character, {}).get("ability") == "first_night_info":
                score += 1
                
            # Prefer targeting protection roles
            if self.character_data.get(target.character, {}).get("ability") == "protection":
                score += 2
        ...
        self._log_decision("demon_kill", 0.7, f"Selected based on role value and game balance", {
            "demon": demon.name,
            "target": target_name,
    ...
    def decide_monk_protection(self, monk: Player, game_state: GameState) -> Optional[str]:
        """Decide who the Monk protects"""
        
        # Get possible targets (alive players except monk)
        possible_targets = [
            p for p in game_state.players 
            if p.is_alive() and p.name != monk.name
        ]
        ...
        for target in possible_targets:
            ...
            if self.character_data.get(target.character, {}).get("ability") in ["ongoing_info", "first_night_info"]:
                score += 2
        ...
        self._log_decision("monk_protection", 0.6, f"Protected most valuable target", {
            "monk": monk.name,
            "target": target_name,
    ...
    def analyze_nomination(self, nominator: str, nominee: str, game_state: GameState) -> Dict[str, Any]:
    ...
    def update_game_balance(self, game_state: GameState):
        ...
        alive_good = len([p for p in game_state.get_alive_players() if self._is_good(p.character)])
        alive_evil = len([p for p in game_state.get_alive_players() if self._is_evil(p.character)])
        ...
        if total_alive > 0:
            ...
            if game_state.day_number == 1:
                ...
                info_roles_alive = len([
                    p for p in game_state.get_alive_players() 
                    if self.character_data.get(p.character, {}).get("ability") in ["ongoing_info", "first_night_info"]
                ])
    ...
    def _log_decision(self, decision_type: str, confidence: float, reasoning: str, data: Dict[str, Any]):
        """Log an AI decision"""
        ...
        self.logger.info(f"AI Decision [{decision_type}]: {reasoning} (confidence: {confidence:.2f})")
    ...
    def get_decision_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_confidence": sum(d.confidence for d in self.decisions_made) / len(self.decisions_made) if self.decisions_made else 0,
