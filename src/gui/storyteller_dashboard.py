...
class StorytellerDashboard:
    ...
    def _create_widgets(self):
        ...
        self.title_label = ttk.Label(
            self.title_frame, text="🎭 AI STORYTELLER DASHBOARD", style="Title.TLabel"
        )
    ...
    def _create_connection_panel(self):
        ...
        conn_frame = ttk.LabelFrame(
            self.left_panel, text="🔗 Platform Connection", style="Dark.TFrame"
        )
        ...
        ttk.Label(platform_frame, text="Platform:", style="Dashboard.TLabel").pack(
            side=tk.LEFT
        )
        ...
        ttk.Label(room_frame, text="Room Code:", style="Dashboard.TLabel").pack(
            side=tk.LEFT
        )
    ...
    def _create_game_control_panel(self):
        ...
        control_frame = ttk.LabelFrame(
            self.left_panel, text="🎮 Game Control", style="Dark.TFrame"
        )
        control_frame.pack(fill=tk.X, padx=5, pady=5)

        # Phase display
        self.phase_label = ttk.Label(
            control_frame, text="Phase: SETUP", style="Phase.TLabel"
        )
        self.phase_label.pack(pady=5)

        # Day counter
        self.day_label = ttk.Label(
            control_frame, text="Day 0", style="Dashboard.TLabel", font=("Segoe UI", 12)
        )
        self.day_label.pack()
        ...
        action_frame = ttk.LabelFrame(
            control_frame, text="Quick Actions", style="Dark.TFrame"
        )
    ...
    def _create_grimoire_panel(self):
        ...
        grimoire_frame = ttk.LabelFrame(
            self.middle_panel, text="📖 Grimoire", style="Dark.TFrame"
        )
        ...
        self.grimoire_canvas = tk.Canvas(
            grimoire_frame, bg="#0a0a0a", highlightthickness=0
        )

        grimoire_scroll = ttk.Scrollbar(
            grimoire_frame, orient=tk.VERTICAL, command=self.grimoire_canvas.yview
        )
        self.grimoire_scrollable = ttk.Frame(self.grimoire_canvas, style="Dark.TFrame")

        self.grimoire_scrollable.bind(
            "<Configure>",
            lambda e: self.grimoire_canvas.configure(
                scrollregion=self.grimoire_canvas.bbox("all")
            ),
        )

        self.grimoire_canvas.create_window(
            (0, 0), window=self.grimoire_scrollable, anchor="nw"
        )
    ...
    def _create_communication_panel(self):
        ...
        comm_frame = ttk.LabelFrame(
            self.right_panel, text="🎤 Communication", style="Dark.TFrame"
        )
        ...
        self.speaker_label = ttk.Label(
            comm_frame, text="🔇 No one speaking", style="Dashboard.TLabel"
        )
        self.speaker_label.pack(pady=5)

        # Speech input/output
        ttk.Label(comm_frame, text="Message to speak:", style="Dashboard.TLabel").pack(
            anchor=tk.W, padx=5
        )
        ...
        # Communication log
        ttk.Label(comm_frame, text="Communication Log:", style="Dashboard.TLabel").pack(
            anchor=tk.W, padx=5
        )
    ...
    def _create_ai_panel(self):
        ...
        ai_frame = ttk.LabelFrame(
            self.right_panel, text="🧠 AI Decisions", style="Dark.TFrame"
        )
        ...
        ttk.Label(mode_frame, text="AI Mode:", style="Dashboard.TLabel").pack(
            side=tk.LEFT
        )
    ...
    def _connect_to_platform(self):
        ...
        asyncio.run_coroutine_threadsafe(
            self._async_connect(platform, room_code), self.event_loop
        )
    ...
    async def _async_connect(self, platform: str, room_code: str):
        ...
        try:
            ...
            if success:
                ...
                # Initialize AI
                self.storyteller_ai = StorytellerAI(
                    self.api_client, self.speech_handler
                )
                ...
                # Initial greeting
                await self._speak(
                    "Good evening, players. I am your AI Storyteller. Let's begin our tale of good versus evil."
                )
    ...
    def _start_night_phase(self):
        ...
        asyncio.run_coroutine_threadsafe(self._async_start_night(), self.event_loop)
    ...
    async def _async_start_night(self):
        ...
        try:
            ...
            await self.api_client.announce_phase_change(
    ...
    async def _handle_fortune_teller(self, player: Player):
        ...
        await self._speak(
            "Fortune Teller, choose two players to learn if one is the Demon."
        )
        ...
        if len(choices) == 2:
            ...
            await self.api_client.give_private_info(
                player.name, f"Fortune Teller result: {response}"
            )
    ...
    async def _handle_empath(self, player: Player):
        ...
        evil_count = self.storyteller_ai.decide_empath_result(
            player, left, right, self.game_state
        )

        # Show fingers
        await self._speak_to_player(
            player.name, f"You sense {evil_count} evil neighbors."
        )
        await self.api_client.give_private_info(
            player.name, f"Empath senses: {evil_count}"
        )

        self._log_ai_decision(
            f"Empath ({player.name}) senses {evil_count} evil neighbors"
        )
    ...
    def _toggle_listening(self):
        ...
        if self.is_listening:
            ...
            asyncio.run_coroutine_threadsafe(self._continuous_listen(), self.event_loop)
    ...
    def _speak_to_all(self):
        ...
        if text:
            asyncio.run_coroutine_threadsafe(self._speak(text), self.event_loop)
    ...
    def _update_grimoire(self):
        ...
        for i, player in enumerate(self.game_state.players):
            ...
            token_frame = tk.Frame(
                self.grimoire_scrollable, bg="#2a2a2a", relief="raised", bd=2
            )
    ...
    def _on_player_click(self, player: Player):
        ...
        menu.add_command(
            label=f"Wake {player.name}", command=lambda: self._wake_player(player)
        )
        menu.add_command(
            label=f"Give Info to {player.name}",
            command=lambda: self._give_info_dialog(player),
        )
        menu.add_separator()

        if player.is_alive():
            menu.add_command(label="Kill", command=lambda: self._kill_player(player))
        else:
            menu.add_command(
                label="Resurrect", command=lambda: self._resurrect_player(player)
            )

        menu.add_separator()
        menu.add_command(label="Make Drunk", command=lambda: self._make_drunk(player))
        menu.add_command(label="Poison", command=lambda: self._poison_player(player))
        menu.add_command(
            label="Add Reminder", command=lambda: self._add_reminder(player)
        )
