...
class ObserverWindow:
    ...
    def _setup_styles(self):
        ...
        # Dark theme
        style.configure('Observer.TLabel', 
                       background='#1a1a1a', 
                       foreground='#ffffff', 
                       font=('Segoe UI', 11))
    ...
    def _create_connection_panel(self):
        ...
        connection_frame = ttk.LabelFrame(self.left_panel, text="📡 Connection", style='Dark.TFrame')
        connection_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # Game URL input
        ttk.Label(connection_frame, text="Game URL:", style='Observer.TLabel').pack(anchor=tk.W, padx=5, pady=2)
        ...
        # Room code input
        ttk.Label(connection_frame, text="Room Code:", style='Observer.TLabel').pack(anchor=tk.W, padx=5, pady=2)
    ...
    def _create_game_state_panel(self):
        ...
        state_frame = ttk.LabelFrame(self.left_panel, text="🎭 Game State", style='Dark.TFrame')
        ...
        self.phase_label = ttk.Label(info_frame, text="Phase: Setup", style='Subtitle.TLabel')
        ...
        self.script_label = ttk.Label(info_frame, text="Script: Unknown", style='Observer.TLabel')
        self.script_label.pack(anchor=tk.W, padx=5)
        
        # Players list
        ttk.Label(state_frame, text="Players:", style='Subtitle.TLabel').pack(anchor=tk.W, padx=5, pady=(10,0))
        ...
        self.players_scrollbar = ttk.Scrollbar(players_container, orient=tk.VERTICAL, command=self.players_canvas.yview)
        ...
        self.players_canvas.create_window((0, 0), window=self.players_scrollable, anchor="nw")
    ...
    def _create_analysis_panel(self):
        ...
        analysis_frame = ttk.LabelFrame(self.right_panel, text="🧠 AI Analysis", style='Dark.TFrame')
        ...
        # Player Analysis tab
        self.player_analysis_frame = ttk.Frame(self.analysis_notebook, style='Dark.TFrame')
        self.analysis_notebook.add(self.player_analysis_frame, text="👥 Players")
        
        # Game Flow tab
        self.flow_analysis_frame = ttk.Frame(self.analysis_notebook, style='Dark.TFrame')
    ...
    def _create_ai_insights_panel(self):
        ...
        insights_frame = ttk.LabelFrame(self.right_panel, text="💭 AI Commentary", style='Dark.TFrame')
        ...
        ttk.Button(
            controls_frame,
            text="🎤 Manual Comment",
            command=self._add_manual_comment
    ...
    async def _observe_game_loop(self):
        ...
        try:
            ...
            self.root.after(0, lambda: self.status_label.config(text="Connected & Observing", foreground='#4CAF50'))
            self.root.after(0, lambda: self._add_commentary("✅ Successfully connected! Observing game..."))
    ...
    async def _analyze_game_state(self):
        ...
        self.root.after(0, lambda: self._update_analysis_display(player_analysis, flow_analysis, predictions))
    ...
    async def _analyze_players(self) -> str:
        ...
        for player in self.game_state.players:
            ...
            analysis.append(f"   Predicted Role: {self.predicted_roles.get(player.name, 'Unknown')}")
            analysis.append("")
    ...
    async def _analyze_game_flow(self) -> str:
        ...
        for nomination in self.game_state.nominations:
            vote_ratio = len(nomination.voters) / len(self.game_state.get_alive_players())
            analysis.append(f"   {nomination.nominee}: {len(nomination.voters)} votes ({vote_ratio:.1%})")
        ...
        for player in dead_players:
            analysis.append(f"   {player.name} - Day {player.private_info.get('death_day', '?')}")
    ...
    def _update_analysis_display(self, player_analysis: str, flow_analysis: str, predictions: str):
    ...
    def _parse_server_game_state(self, data: Dict[str, Any]) -> GameState:
        ...
        for i, player_data in enumerate(data.get('players', [])):
            player = Player(
                ...
                status=PlayerStatus.ALIVE if player_data.get('alive', True) else PlayerStatus.DEAD
            )
    ...
    async def _process_phase_change(self, data: Dict[str, Any]):
        ...
        self.root.after(0, lambda: self._add_commentary(f"🌅 Phase changed to: {new_phase}"))
    ...
    async def _process_nomination(self, data: Dict[str, Any]):
        ...
        self.root.after(0, lambda: self._add_commentary(f"⚖️ {nominator} nominates {nominee}"))
    ...
    async def _process_vote(self, data: Dict[str, Any]):
        ...
        self.root.after(0, lambda: self._add_commentary(f"🗳️ {voter} votes for {nominee}"))
        
    async def _process_execution(self, data: Dict[str, Any]):
        """Process execution event"""
        executed = data.get('executed')
        
        self.root.after(0, lambda: self._add_commentary(f"💀 {executed} has been executed"))
        
    async def _process_night_action(self, data: Dict[str, Any]):
        """Process night action event"""
        character = data.get('character')
        
        self.root.after(0, lambda: self._add_commentary(f"🌙 {character} acts during the night"))
