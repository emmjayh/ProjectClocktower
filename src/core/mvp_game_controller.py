...
class MVPGameController:
    ...
    def __init__(self, use_mock_choices: bool = False):
        ...
        self.choice_system = MockPlayerChoices() if use_mock_choices else SimpleCommandLineChoices()
    ...
    async def _wake_character(self, character: str, is_first_night: bool):
        ...
        players_with_char = [
            p for p in self.engine.game_state.players 
    ...
    async def _handle_fortune_teller(self, player: Player):
        ...
        valid_targets = [p.name for p in self.engine.get_alive_players() if p.name != player.name]
