...
from .game_persistence import GamePersistence, AutoSaveManager
from .game_replay import GameRecorder, GamePlayer, ReplayManager, EventType
...
class GameAutomation:
    ...
    def __init__(self, game_state: GameState, rule_engine: RuleEngine, 
                 speech_handler: SpeechHandler, live_monitor: LiveGameMonitor = None):
    ...
    def _define_transitions(self):
        ...
        self.transitions = [
            ...
            # First Night to Dawn
            PhaseTransition(
                GamePhase.FIRST_NIGHT_INFO, GamePhase.DAWN,
            ...
            PhaseTransition(
                GamePhase.DAWN, GamePhase.DAY_DISCUSSION,
            ...
            # Day to Nominations
            PhaseTransition(
                GamePhase.DAY_DISCUSSION, GamePhase.NOMINATIONS,
            ...
            PhaseTransition(
                GamePhase.NOMINATIONS, GamePhase.VOTING,
            ...
            # Night to Dawn
            PhaseTransition(
                GamePhase.NIGHT_ACTIONS, GamePhase.DAWN,
                wait_before=WaitCondition(WaitReason.DRAMATIC_PAUSE, min_duration=2.0),
                announcement="The night is over. Everyone, open your eyes."
            ),
        ]
        ...
        try:
            while self.current_phase != GamePhase.GAME_OVER:
                ...
                await asyncio.sleep(0.1)
                    ...
                    await self._announce("I encountered an error. Please check the logs.")
                    break
    ...
    def _setup_live_monitor_callbacks(self):
        ...
        self.live_monitor.register_event_callback("nomination_received", self._on_live_nomination)
        self.live_monitor.register_event_callback("vote_received", self._on_live_vote)
        self.live_monitor.register_event_callback("close_nominations_request", self._on_close_nominations)
        self.live_monitor.register_event_callback("start_voting_request", self._on_start_voting)
    ...
    async def _on_live_nomination(self, data: Dict[str, Any]):
        ...
        if self.current_phase == GamePhase.NOMINATIONS and nominee:
            ...
            await self._announce(f"Nomination received: {nominator} nominates {nominee}")
        ...
        if self.current_phase == GamePhase.VOTING and self.current_vote:
            ...
            if vote in ["yes", "aye"]:
                if voter not in self.current_vote.get("votes", {}).get("yes", []):
                    ...
                    # Record vote
                    if self.recording_enabled:
                        self.recorder.record_vote(voter, "yes", self.current_vote.get("nominee"))
                ...
                if voter not in self.current_vote.get("votes", {}).get("no", []):
                    ...
                    # Record vote
                    if self.recording_enabled:
                        self.recorder.record_vote(voter, "no", self.current_vote.get("nominee"))
    ...
    def _check_game_over(self) -> bool:
        ...
        alive_good = sum(1 for p in self.game_state.get_alive_players() 
                        if p.team == "good")
        alive_evil = sum(1 for p in self.game_state.get_alive_players() 
                        if p.team == "evil")
        ...
        demons_alive = sum(1 for p in self.game_state.get_alive_players() 
    ...
    def _get_night_order(self) -> List[str]:
        ...
        night_order = [
            "Poisoner", "Monk", "Scarlet Woman", "Imp", 
            "Ravenkeeper", "Fortune Teller", "Butler", "Empath", "Spy"
        ]
    ...
    async def _wake_role_first_night(self, role: str):
        ...
        players_with_role = [p for p in self.game_state.get_alive_players() 
                            if p.character == role]
    ...
    async def _is_vote_complete(self) -> bool:
        ...
        total_votes = len(self.current_vote.get("votes", {}).get("yes", [])) + \
                     len(self.current_vote.get("votes", {}).get("no", []))
        alive_players = len(self.game_state.get_alive_players())
    ...
    async def _announce_vote_result(self, result: Dict[str, Any]):
        ...
        if result.get("passed"):
            await self._announce(
                f"The vote passes with {yes_votes} votes. "
    ...
    async def _execute_player(self, player_name: str):
        ...
        if player and player.is_alive():
            ...
            if self.recording_enabled:
                ...
                yes_votes = len(self.current_vote.get("votes", {}).get("yes", [])) if self.current_vote else 0
                alive_players = len(self.game_state.get_alive_players())
                threshold = (alive_players // 2) + 1
    ...
    def _get_info_roles(self) -> List[str]:
        ...
        return [
            "Fortune Teller", "Empath", "Chef", "Washerwoman", 
            "Librarian", "Investigator", "Spy"
        ]
    ...
    def _process_night_deaths(self):
        ...
        if deaths:
            death_announcement = "During the night, the following players died: " + ", ".join(deaths)
    ...
    async def _reveal_all_roles(self):
        ...
        for player in self.game_state.players:
            ...
            await self._announce(f"{player.name}: {player.character} ({player.team}) - {status}")
    ...
    async def _generate_spy_info(self) -> str:
        ...
        for player in self.game_state.players:
            ...
            info_parts.append(f"  {player.name}: {player.character} ({team}) - {status}")
    ...
    def get_automation_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "live_monitoring": self.live_monitor.listening_active if self.live_monitor else False
        }
    ...
    async def _execute_character_ability(self, player: Player, trigger: TriggerType, role: str):
            ...
            if execution:
                ...
                if self.recording_enabled:
                    self.recorder.record_ability_execution(
                        player.name, role, targets, 
                        execution.result.name, execution.effects
                    )
    ...
    async def _get_ability_targets(self, role: str, trigger: TriggerType) -> Optional[List[str]]:
    ...
    async def _check_nomination_abilities(self, nominator: str, nominee: str):
        ...
        if nominee_player and nominee_player.character == "Virgin":
            ...
            execution = await self.ability_system.execute_ability(
                "Virgin", nominee_player, self.game_state, TriggerType.ON_NOMINATION,
                nominator=nominator
            )
