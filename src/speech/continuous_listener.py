...
class ListenerConfig:
    ...
    wake_words: List[str] = field(default_factory=lambda: [
        ...
        "storyteller", "demon", "minion", "outsider", "townsfolk"
    ])
...
class VoiceActivityDetector:
    ...
    def __init__(self, config: ListenerConfig):
        ...
        self.min_speech_frames = int(config.min_speech_duration * config.sample_rate / config.chunk_size)
        self.max_silence_frames = int(config.max_silence_gap * config.sample_rate / config.chunk_size)
...
class ContinuousListener:
    ...
    def _audio_callback(self, in_data, frame_count, time_info, status):
        ...
        if self.listening:
            ...
            if self.vad.process_chunk(in_data):
                ...
                try:
                    audio_window = self.audio_buffer.get_window(self.config.window_duration)
    ...
    def _parse_command(self, text: str, wake_words: List[str]) -> Optional[Dict[str, Any]]:
